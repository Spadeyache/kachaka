...
public:
    MotorPublisher() : Node("motor_publisher"), state_(0)
    {
        ...
        motion_timer_B = this->create_wall_timer(100ms, std::bind(&MotorPublisher::motion_callback_B, this));

        // Timer 2: info logger every 1s
        info_timer_B = this->create_wall_timer(1.5s, [this]() {
            RCLCPP_INFO(this->get_logger(), "nodegit _loop");
            state_ += 1;
        });
    }
...
private:
    ...
    rclcpp::TimerBase::SharedPtr motion_timer_B;
    rclcpp::TimerBase::SharedPtr info_timer_B;
};
